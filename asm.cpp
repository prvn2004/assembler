...
void remove_comments(string &inputLine)
{
    int index = 0;
...
bool isValidLabel(string &givenLabel)
{
    // Check if the label starts with a letter or underscore
    char firstChar = givenLabel[0];
    if (!isalpha(firstChar) && firstChar != '_')
    {
        return false;
    }
    // Check if the remaining characters are alphanumeric or underscore
    for (int i = 1; i < givenLabel.length(); i++)
    {
        char currentChar = givenLabel[i];
...
void insertMachineCode(int nop, vector<string> word, ofstream *list)
{
    ...
    if (nop == 0)
    {
        ...
        *list << setw(8) << setfill('0') << hex << without_oper_map[word[0]] << " ";
        stringstream ss;
        ss << std::setw(8) << std::setfill('0') << std::hex << without_oper_map[word[0]] << " ";
        string output_string = ss.str();
        Object_File.push_back(output_string);
    ...
    else
    {
        ...
        *list << setw(8) << setfill('0') << hex << code << " ";
        stringstream ss;
        ss << std::setw(8) << std::setfill('0') << std::hex << code << " ";
        string output_string = ss.str();
        Object_File.push_back(output_string);
    }
}
...
int main(int argc, char **argv)
{
    ...
    // Append ".obj" extension to the filename
